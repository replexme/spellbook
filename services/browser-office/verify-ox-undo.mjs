...
const runtimeDirectory =
  process.argv[process.argv.indexOf("--candidate-runtime") + 1];
const outputRoot = path.resolve(
  process.argv[process.argv.indexOf("--output") + 1] ??
    "artifacts/browser-office/ox-undo",
);
const measureFlag = process.argv.indexOf("--measure-fixture");
const measureFixture = measureFlag >= 0 ? process.argv[measureFlag + 1] : null;
if (
...
const fixture = new Uint8Array(
  await readFile(
    measureFixture ?? path.join(root, "eval/public/downloads/ox-typical.pptx"),
  ),
);
if (!measureFixture)
  assert.deepEqual(
    Object.keys(unzipSync(fixture)).filter((name) =>
...
const report = {
  input: measureFixture ? "synthetic-performance-deck" : "ox-typical.pptx",
  sourceMasters: measureFixture ? null : 1,
  cycles: [],
};
...
async function waitEvent(page, match) {
  await page.waitForFunction(
    (expected) =>
      globalThis.__oxHost.events.some(
        (event) =>
          event.type === "error" ||
  ...
  const result = await page.evaluate(
    (expected) =>
      globalThis.__oxHost.events.find((event) =>
        Object.entries(expected).every(([key, value]) => event[key] === value),
      ) ?? globalThis.__oxHost.events.find((event) => event.type === "error"),
    match,
  );
  if (!result || result.type === "error" || result.error)
    throw new Error(String(result?.error ?? "product_host_error"));
  return result;
}
async function task(page, request) {
  const id = crypto.randomUUID();
  await page.evaluate(
    ({ id, request }) => globalThis.__oxHost.port.postMessage({ id, request }),
    { id, request },
  );
  ...
  try {
    ...
    await page.evaluate(
      ({ bytes, index }) => {
        ...
        globalThis.__oxHost.port.postMessage(
          {
            type: "open",
            requestId: `open-${index}`,
            documentId: `ox-undo-${index}`,
            fileName: "ox-typical.pptx",
            revision:
    ...
    assert.equal(
      changed.slides[0].elements.find(
    ...
    if (measureFixture) {
      ...
      assert.equal(reviewed.revision, changed.revision);
      return {
        status: "passed",
        slideCount: before.slides.length,
        editAndReviewMs: Date.now() - editStartedAt,
        readMetrics: {
          initial: before.readMetrics,
          edit: changed.readMetrics,
          afterEdit: reviewed.readMetrics,
        },
      };
    }
    ...
    await page.waitForTimeout(25_000);
    const typed = await task(page, {
      operation: "observe",
      captureSlideIndexes: [],
    });
    assert.match(
      typed.slides[0].elements.find(
        (element) => element.elementId === target.elementId,
      ).text,
      /AI 수정 사람/,
    );
    ...
    await page.waitForFunction(
      (prior) =>
        globalThis.__oxHost.events
          .slice(prior)
          .some((event) => event.type === "save" || event.type === "error"),
      previous,
      { timeout },
    );
    const saved = await page.evaluate(
      (prior) =>
        globalThis.__oxHost.events
          .slice(prior)
          .find((event) => event.type === "save" || event.type === "error"),
      previous,
    );
    ...
    const bytes = await page.evaluate(
      (requestId) =>
        Array.from(
          new Uint8Array(
            globalThis.__oxHost.events.find(
              (event) => event.type === "save" && event.requestId === requestId,
            ).bytes,
          ),
        ),
      saved.requestId,
    ...
    return {
      ...
      readMetrics: {
        initial: before.readMetrics,
        edit: changed.readMetrics,
        afterTyping: typed.readMetrics,
        afterUndo: once.readMetrics,
...
if (report.cycles.some((cycle) => cycle.status !== "passed"))
  process.exitCode = 1;
