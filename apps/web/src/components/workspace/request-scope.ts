...
export function scopeDescription(
  ...
  if (count === 1)
    return `지금 선택한 ${selectedLabel(selection)} 1개만 바꿔요`;
 ...
 */
export const ALWAYS_EDITABLE = new Set([
  "replace_text",
  "move",
  "resize",
  "align",
...
export function suggestionsFor(
  ...
  const editable = (operation: string) =>
    ALWAYS_EDITABLE.has(operation) ||
    Boolean(selection?.editableOperations?.includes(operation));
  ...
  if (selected.length === 1) {
    ...
    if (/Graphic|Picture/i.test(kind)) {
      ...
      if (label === "제목" && editable("font_size"))
        items.push({
          text: "다른 슬라이드 제목과 글자 크기 맞추기",
          scope: "document",
        });
    }
    if (items.length)
      groups.push({
        title: `선택한 ${label}${directionParticle(label)}`,
        items,
      });
  } else if (selection) {
    const items: Suggestion[] = [
      { text: "이 슬라이드의 맞춤법 고치기", scope: "slides" },
    ];
    if (editable("font_size"))
      items.push({
        text: "이 슬라이드 본문의 글자 크기 맞추기",
        scope: "slides",
      });
    groups.push({
      title: `${selection.activeSlide + 1}번 슬라이드에서`,
      items,
    });
  }
  groups.push({
