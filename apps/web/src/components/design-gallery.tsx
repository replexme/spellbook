...
function useSampleSlide(variant: "before" | "after") {
  ...
  useEffect(() => {
    ...
    ["국내 시장 규모 4.2조 원", "상위 3사 점유율 61%", "중소 고객 비중 확대"].forEach((line, index) =>
...
const baseSummary: TurnSummary = {
  ...
  slideGroups: [{ slideIndex: 2, count: 2, targets: [{ label: "제목", count: 1 }, { label: "본문 3줄", count: 1 }] }],
  sharedDetail: null,
};
...
const multiSummary: TurnSummary = {
  ...
  changes: [
    { slideIndex: 0, target: "부제목", kind: "modified", details: ["문구 “2026.9.18” → “2026년 9월 18일”"], box: null },
    { slideIndex: 2, target: "바닥글", kind: "modified", details: ["문구 “2026.9.18” → “2026년 9월 18일”"], box: null },
    { slideIndex: 3, target: "바닥글", kind: "modified", details: ["문구 “2026.9.18” → “2026년 9월 18일”"], box: null },
    { slideIndex: 4, target: "텍스트 상자", kind: "modified", details: ["문구 “9.18” → “9월 18일”"], box: null },
  ],
  unchangedSlides: 8,
  undoSteps: 1,
  evidence: [0, 2, 3, 4].map((slideIndex) => ({ slideIndex, before: "before", after: "after" })),
  slideGroups: [
    ...
    { slideIndex: 4, count: 4, targets: [{ label: "텍스트 상자", count: 3 }, { label: "바닥글", count: 1 }] },
  ],
  sharedDetail: null,
};

function turn(overrides: Partial<CardTurn>, summary: Partial<TurnSummary> | null = {}): CardTurn {
  return {
    key: "sample",
    turnId: "sample",
...
export function DesignGallery() {
  ...
  const pairsOf = (sample: CardTurn, source: EvidencePair["source"] = "ai"): EvidencePair[] =>
  ...
  const cards: Array<[string, CardTurn, { source?: EvidencePair["source"]; undo?: UndoAction | null }?]> = [
    ["바뀜 · AI가 다시 보고 검토함", turn({})],
    [
      "바뀜 · 여러 슬라이드",
      turn({ requestText: "모든 날짜를 ‘2026년 9월 18일’ 형식으로 바꿔 줘", permission: "document" }, multiSummary),
      { undo: { kind: "native", label: "모두 되돌리기" } },
    ],
    [
      "바뀜 · 다시 확인하지 못함",
      turn({ reviewed: false }, { outcome: "unverified", reviewed: false, introducedIssues: { overlap: 1, outOfBounds: 0, invalidSize: 0 } }),
      { undo: { kind: "restore", label: "이 요청 전으로 돌아가기" } },
    ],
    ["새로고침 뒤 · 저장본 미리보기", turn({}), { source: "saved", undo: { kind: "restore", label: "이 요청 전으로 돌아가기" } }],
    ["되돌림", turn({ undoneAt: new Date().toISOString() }), { undo: null }],
    [
      "고치지 못함 · 범위 밖",
      turn({ changed: false, permission: "selection" }, { outcome: "unchanged", changes: [], changedSlides: [], evidence: [], scopeRejected: true }),
    ],
    [
      "바뀐 것 없음",
      turn({ changed: false, permission: "slides" }, { outcome: "unchanged", changes: [], changedSlides: [], evidence: [] }),
    ],
    ["답만 함", turn({ changed: false, text: "3번 슬라이드는 제목 1개, 본문 3줄, 막대그래프 1개로 되어 있어요." }, { outcome: "answered", changes: [], evidence: [] })],
    [
      "고치지 못함",
      turn(
        { status: "error", changed: false },
        { outcome: "failed", changes: [], evidence: [], failure: { code: "usage_limit", message: "구독 사용 한도에 닿았어요. 잠시 뒤 다시 요청해 주세요." } },
      ),
    ],
    ["중단됨", turn({ status: "error", changed: false }, { outcome: "cancelled", changes: [], evidence: [], changedSlides: [], failure: { code: "cancelled", message: "작업을 중단했어요." } })],
  ];
  return (
    <main className="app-main gallery">
      <header className="gallery-head">
        <Brand />
        <h1>디자인 시스템</h1>
        <p>토큰 → 부품 → 패턴 → 화면. 모든 화면은 여기 있는 것만으로 만듭니다. 예시 데이터만 씁니다.</p>
      </header>
      ...
      <Section title="색 역할">
        ...
        <p className="dialog-note">청록(AI가 한 일)은 AI의 결과와 AI로 보내는 동작에만 씁니다.</p>
      </Section>

      <Section title="글자">
        <div className="gallery-stack">
          {sizes.map((size) => (
            <p key={size} style={{ margin: 0, fontSize: `var(--ds-text-${size})` }}>
              {size} · 요청 하나에 카드 한 장
            </p>
          ))}
        </div>
      ...
      <Section title="선택">
        ...
        <div className="gallery-row">
          <TextField label="파일 이름" defaultValue="2026 하반기 사업계획" hint="끝의 .pptx는 저절로 붙어요" />
        </div>
      </Section>
      ...
      <Section title="상태 표시">
        ...
        <div className="gallery-stack">
          ...
          <Banner tone="danger">구독 사용 한도에 닿았어요. 잠시 뒤 다시 요청해 주세요.</Banner>
      ...
      <Section title="확인 목록과 진행 단계">
        <div className="gallery-columns">
          <CheckList
            label="확인 목록"
            items={[
              { tone: "ok", label: "바뀐 화면을 AI가 다시 보고 검토함", evidence: "turn.reviewed" },
              { tone: "warn", label: "새로 생긴 겹침 1곳", evidence: "edit.layoutAudit.introducedIssues" },
              { tone: "fail", label: "저장한 파일을 다시 열지 못함", evidence: "sample" },
              { tone: "na", label: "PowerPoint에서 직접 열어 보는 확인은 아직 하지 않아요", evidence: "sample" },
              { tone: "info", label: "6번 슬라이드의 SmartArt 1개는 AI가 고치지 못해요", evidence: "sample" },
            ]}
          />
          <StepList
            label="진행 단계"
      ...
      <Section title="슬라이드 그림">
        <div className="gallery-columns">
          <SlideImage src={after} alt="예시 슬라이드" marks={baseSummary.changes.map((change) => ({ ...change.box!, label: change.target }))} />
      ...
      <Section title="결과 카드">
        <div className="gallery-cards">
          <div>
            <h3>작업 중</h3>
            <RunningCard
              turn={turn({ status: "running", tools: ["현재 슬라이드 확인", "수정 계획 검사", "여러 요소 한 번에 수정"], finishedAt: null }, null)}
          ...
          {cards.map(([title, sample, options]) => (
            <div key={title}>
              ...
              <ResultCard
                ...
                undo={options && "undo" in options ? (options.undo ?? null) : undo}
                onUndo={noop}
                onCompare={noop}
                onRetry={noop}
                onReveal={noop}
      ...
      <Section title="대화 줄">
        <div className="gallery-stack">
          <ConversationLog
            items={[
              { kind: "user", key: "u", text: "3번 슬라이드 제목을 한 줄로 줄여 줘", permission: "slides", queued: false },
              { kind: "manual", key: "m", run: { from: sampleTime(10, 31), to: sampleTime(10, 38), saves: 3, afterTurnId: null } },
              { kind: "user", key: "q", text: "표의 숫자 서식을 천 단위 쉼표로 통일해 줘", permission: "slides", queued: true },
            ]}
            renderTurn={() => null}
          />
          <div className="gallery-row">
            <ScopeMenu value="selection" onChange={noop} selection={sampleSelection} />
            <ScopeMenu value="slides" onChange={noop} selection={sampleSelection} />
            <ScopeMenu value="selection" onChange={noop} selection={{ ...sampleSelection, selected: [] }} />
          </div>
        </div>
      </Section>
      ...
      <Section title="휴대폰 보기와 여는 중">
        <div className="gallery-columns">
          <div className="gallery-phone">
            <PhoneSlides previews={[before, after, after]} index={1} onIndex={noop} />
          </div>
          <div className="gallery-frame">
            <OpeningView previews={[before, after, after, before]} message="편집기 준비 중 · 처음 열 때는 1분쯤 걸려요" />
          </div>
        </div>
      </Section>
