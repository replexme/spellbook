...
function observe(id: string, slides = baseSlides, imageSlide = 2): TurnTaskRecord {
  return {
    id,
    request: { operation: "observe" },
    status: "completed",
    result: { slides, images: [{ slideIndex: imageSlide, pngBase64: png }], changedSlideIndexes: [] },
    error: null,
  };
}

function batch(id: string, overrides: Record<string, unknown> = {}): TurnTaskRecord {
  return {
    id,
...
describe("summarizeTurn", () => {
  it("summarizes a reviewed change from the editor records", () => {
    ...
    expect(summary.changes[0]).toMatchObject({ slideIndex: 2, target: "제목", kind: "modified" });
    ...
    expect(summary.introducedIssues).toEqual({ overlap: 0, outOfBounds: 0, invalidSize: 0 });
  ...
  it("marks a change the AI did not re-check as unverified", () => {
    const summary = summarizeTurn({ ...completed, reviewed: false }, [observe("t1"), batch("t2")]);
  ...
  it("marks whole-window captures from the browser editor so outlines are not placed on them", () => {
    const windowed = (task: TurnTaskRecord): TurnTaskRecord => ({
      ...
      result: {
        ...task.result!,
        images: (task.result!.images as Array<Record<string, unknown>>).map((image) => ({
          ...image,
          source: "browser_canvas",
        })),
      },
    });
    const summary = summarizeTurn(completed, [windowed(observe("t1")), windowed(batch("t2"))]);
    expect(summary.evidence[0]!.framing).toBe("window");
  });

  it("follows the editor record when nothing changed, whatever the reply says", () => {
    const summary = summarizeTurn({ ...completed, changed: false, reviewed: false }, [
      observe("t1"),
      batch("t2", { changedSlideIndexes: [], transaction: { status: "unchanged" } }),
    ]);
  ...
  it("treats a read-only request as an answer", () => {
    const summary = summarizeTurn(
      { ...completed, permissionMode: "read_only", changed: false, reviewed: false },
      [observe("t1")],
    );
  ...
  it("reports scope rejections from failed editor tasks", () => {
    const summary = summarizeTurn({ ...completed, changed: false, reviewed: false }, [
      observe("t1"),
      { id: "t2", request: { operation: "edit" }, status: "failed", result: null, error: "선택 범위 밖입니다." },
    ]);
    expect(summary.outcome).toBe("unchanged");
  ...
  it("counts introduced layout issues once", () => {
    const issue = { code: "out_of_slide_bounds", slideIndex: 2, stableIds: ["s2-title"] };
    const summary = summarizeTurn(completed, [
      observe("t1"),
      batch("t2", { layoutAudit: { introducedIssueCount: 1, introducedIssues: [issue] } }),
      batch("t3", { layoutAudit: { introducedIssueCount: 1, introducedIssues: [issue] } }),
    ]);
    expect(summary.introducedIssues).toEqual({ overlap: 0, outOfBounds: 1, invalidSize: 0 });
  });

  it("does not guess element changes when slides were added", () => {
    const summary = summarizeTurn(completed, [
      observe("t1"),
      batch("t2", { slides: [...editedSlides, slide(3, [])], changedSlideIndexes: [3] }),
    ]);
    expect(summary.changes).toEqual([
      expect.objectContaining({ target: "문서", details: ["슬라이드 3장 → 4장"] }),
    ]);
  });
  ...
  it("finds a change on a slide the edit did not report", () => {
    const touched = [slide(0, [{ ...body, elementId: "0/0", stableId: "s0-body", text: "바뀜" }]), ...editedSlides.slice(1)];
    const before = [slide(0, [{ ...body, elementId: "0/0", stableId: "s0-body" }]), ...baseSlides.slice(1)];
    const summary = summarizeTurn(completed, [observe("t1", before), batch("t2", { slides: touched })]);
  ...
  it("makes no claim about other slides when the masters changed", () => {
    const summary = summarizeTurn(completed, [
      { ...observe("t1"), result: { ...observe("t1").result!, masters: [{ name: "A", shapeCount: 3 }] } },
  ...
  it("reports moved slides as a structural change, not element edits", () => {
    ...
    const summary = summarizeTurn(completed, [
      observe("t1"),
      {
        ...
        request: { operation: "edit_batch", commands: [{ op: "move_slide", slideIndex: 2, toIndex: 1 }] },
      },
    ]);
    expect(summary.changes).toEqual([
      expect.objectContaining({ target: "문서", details: ["슬라이드 순서 바꿈"] }),
    ]);
  ...
  it("measures from the first edit that took effect", () => {
    ...
    const personEdited = [slide(0, []), slide(1, [{ ...body, elementId: "1/0", stableId: "s1-body" }]), slide(2, [title, body])];
  ...
  it("adds up the undo actions of every applied edit", () => {
    ...
    const summary = summarizeTurn(completed, [observe("t1"), batch("t2"), single]);
    expect(summary.undoSteps).toBe(2);
  });

  it("groups changes by slide and finds the one detail they share", () => {
    const retitled = (index: number) => ({ ...title, elementId: `${index}/0`, stableId: `s${index}-title` });
    const before = [0, 1, 2].map((index) => slide(index, [retitled(index)]));
    const after = [0, 1, 2].map((index) => slide(index, [{ ...retitled(index), fontSize: 28 }]));
    const summary = summarizeTurn(completed, [observe("t1", before), batch("t2", { slides: after, changedSlideIndexes: [0, 1, 2] })]);
  ...
  it("flags an after image the browser editor may not have repainted", () => {
    const stale = batch("t2", { images: [{ slideIndex: 2, pngBase64: png, stale: true }] });
    const summary = summarizeTurn({ ...completed, reviewed: false }, [observe("t1"), stale]);
    expect(summary.evidence[0]!.stale).toBe(true);
  });

  it("explains failures with a readable reason", () => {
    const summary = summarizeTurn(
      { ...completed, status: "failed", changed: false, reviewed: false, lastError: "native_agent_interrupted" },
      [observe("t1")],
    );
...
describe("labels and reasons", () => {
  it("names elements in Korean", () => {
    ...
    expect(elementLabel({ name: "", kind: "com.sun.star.drawing.GraphicObjectShape" })).toBe("그림");
  });
  ...
  it("maps failure text to specific reasons", () => {
    expect(nativeFailureReason("request timed out after 25s").code).toBe("timeout");
    expect(nativeFailureReason("429 Too Many Requests: rate limit").code).toBe("usage_limit");
    expect(nativeFailureReason("Generated an accurate plan").code).toBe("unknown");
    expect(nativeFailureReason("selected_model_unavailable").code).toBe("model_unavailable");
    expect(nativeFailureReason("native_session_not_active").code).toBe("save_in_progress");
  });
});
